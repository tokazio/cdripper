    ...
    protected FMTChunk(DSDStream ds) throws DecodeException {
        try {
            if (ds.readInt(true) != signature)
                throw new DecodeException("Invalid signature for the chunk 'fmt '", null);
            chunkSize = ds.readLong(false);
            if (chunkSize < 52 || chunkSize > 256)
                throw new DecodeException("Size of the chunk fmt " + chunkSize + " looks out of spec", null);
            chunkSize -= (4 + 8);
            ...
            chunkSize -= 4;
            if (chunkSize % 4 != 0)
                throw new DecodeException("Reserve isn't matching 4 bytes boundary " + chunkSize, null);
