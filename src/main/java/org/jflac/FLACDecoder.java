...
public class FLACDecoder {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(FLACDecoder.class);
    ...
    public FLACDecoder(InputStream bitStream) {
        load(bitStream);
    }
    ...
    public FLACDecoder(String filename) throws IOException {
        load(filename);
    }
    ...
    public ByteData decodeFrame(Frame frame, ByteData pcmData) {
        ...
        switch (streamInfo.getBitsPerSample()) {
            case 8:
    ...
    public void pause() {
        paused = true;
        LOGGER.info("Decoder paused");
    }

    public void resume() {
        paused = false;
        LOGGER.info("Decoder resumed");
    }
     ...
     * @throws IOException  On read error
     */
    public void decode() throws IOException, Throwable {
        readMetadata();
        try {
            while (canContinue) {
                if (!paused) {
    ...
    public void onEnd(final Callback cb) {
        this.onEnd = cb;
    }
    ...
    public long seek(long samplesAbsolute) throws IOException {
        ...
        Map<String, Object> seekBackwardsResults = new HashMap<>();
